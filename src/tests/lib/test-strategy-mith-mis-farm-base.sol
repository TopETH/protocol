...
contract StrategyMithMisFarmTestBase is DSTestSushiBase {
    address want;
    address token1;
    ...
    uint256 performanceInitiatorFee = 100;
    uint256 performanceStrategistFee = 200;
    ...
    function _test_withdraw_release() internal {
        ...
        IERC20(want).safeApprove(address(misJar), 0);
        IERC20(want).safeApprove(address(misJar), _want);
        misJar.deposit(_want);
        misJar.earn();
    ...
    function _test_get_earn_harvest_rewards() internal {
        ...
        uint256 _before = misJar.balance();
